...
double Kp = 0.8;     // 0.8
double Kp_brake = 0.8;     // 0.8
double Kp_acc = 0.4;     // 0.8 
double error;
double P = 0;
double v_internal;
void calc_speed();
...
void dist_cb(const std_msgs::Int64& dist_msg)
{
 dist_rel = dist_msg.data;
 new_data = true;
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_dist = nh.subscribe("distance_data", 1000, dist_cb);
  ...
  while (ros::ok())
  {
   if(new_data){
     ...
     // ROS_INFO("Looping");
      ...
      dist = -dist_rel*0.00562;
	    std::cout << "distance_rel_m " << dist << std::endl;
	    calc_torque();
      calc_speed();
      std_msgs::UInt16 pwm;
