...
//  ros::Subscriber switchSub = nh.subscribe("solver", 1000, &switch_callback);
  ...
  ros::Publisher pub_current = nh.advertise<std_msgs::UInt16>("current_pub", 1000);
  
  ros::Publisher pub_force_qp = nh.advertise<std_msgs::UInt16>("force_qp_pub", 1000);
...
void calc_torque()
{
    ...
    if (std::fabs(y) > 1.4e-5)
    {
      ...
      double psi1 = (2*y)/m;
     	...
     	double bar = (-1/2).*c.^(-1).*g.^(-1).*(v_desired+(-1).*v_internal).^2+(-1).*d.*v_internal+dist;
	    double B2 = 1/bar;
	    double LfB2 = (-4).*c.*g.*m.^(-1).*(c.*g.*(d.*F_r+m.*(v_desired+(-1).*v_internal))+F_r.*((-1).*v_desired+v_internal)).*((v_desired+(-1).*v_internal).^2+2.*c.*g.*(d.*v_internal+(-1).*dist)).^(-2);

	    double LgB2 = 4.*c.*g.*m.^(-1).*(c.*d.*g+(-1).*v_desired+v_internal).*((v_desired+(-1).*v_internal).^2+2.*c.*g.*(d.*v_internal+(-1).*dist)).^(-2);

	    double Distcalc = 2*dist-v_internal;
      double Bcalc = 1/(Distcalc);
    
      double LfB = - (F_r + 2*m*(v_desired - v_internal))/(m*(v_internal-2*dist)^2);
      double LgB = 1/(m*(v_internal-2*dist)^2);
 ...
 case 1:  */
      ...
      std::cout << "Running with full confort constraints" << std::endl;
          int nvar = 3, nineq = 3, neq = 0;
          MatrixXd H(nvar, nvar), Aiq(nineq, nvar), Aeq(neq, nvar);
          VectorXd f(nvar), biq(nineq), beq(neq);
