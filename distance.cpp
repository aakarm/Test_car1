...
void calc_torque()
{
	...
	double F_r = 0.1*(0.1 + 5*v_internal + 0.25*v_internal);
	double m = 9.07;
	double ep = 10;
	double y = v_internal - v_desired;
	double dist = dist_1 - dist_0;
    ...
    /// 1/x Barrier function
      ...
      double LfB = - (F_r + 2*m*(v_desired - v_internal))/(m*(v_internal-2*dist)*(v_internal-2*dist));
      double LgB = 1/(m*(v_internal-2*dist)*(v_internal-2*dist));
